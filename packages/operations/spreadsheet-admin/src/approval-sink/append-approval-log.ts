...
import { APPROVAL_LOG_HEADERS, toApprovalLogSheetRow, type ApprovalLogRecord } from './approval-log-schema'

export interface AppendApprovalLogOptions {
...
function assertHeaderRow(row: string[]): void {
  const normalizedHeader = row.slice(0, APPROVAL_LOG_HEADERS.length)
...
async function ensureApprovalLogSheetHeader(
  client: GoogleSheetsClient,
  spreadsheetId: string,
  ...
  await client.values.batchUpdate({
    ...
    data: [
      {
        range: headerRange,
        values: [APPROVAL_LOG_HEADERS.map(header => header)],
      },
    ],
  })
}

export async function appendApprovalLogEntry(options: AppendApprovalLogOptions): Promise<void> {
  const parsedRange = parseColumnRange(options.range)
  if (!parsedRange) {
    throw new Error(`Unsupported approval log range: ${options.range}`)
  }

  await ensureApprovalLogSheetHeader(options.client, options.spreadsheetId, parsedRange)
  const row = toApprovalLogSheetRow(options.record)
