...
- 이 문서는 phase 1 도입 설계 기준이다.
- phase 1 구현 현황은 `authjs-admin-ui-phase-1-checklist.md`
- 다중 관리자/통계/질문 수정 확장은 `authjs-admin-ui-phase-2-expansion-plan.md`
...
위 비범위 항목은 phase 1 기준이며, 이후 확장 작업은 별도 phase 2 계획 문서에서 다룬다.
...
다만 관리자 수가 2명 이상으로 늘고 수정/배포 액션이 들어가면, 다음 단계에서는 allowlist 위에 role 모델이 추가되어야 한다.
...
세부 계획은 `authjs-admin-ui-phase-2-expansion-plan.md`를 따른다.
...
phase 1 이후에는 아래 순서로 확장한다.

1. 실제 Google OAuth 운영값 등록과 수동 검증 완료
2. 다중 관리자 role 모델 정리
3. 결과 통계 화면 추가
4. 질문 draft / publish 워크플로 도입
5. audit log와 운영 변경 절차 추가

상세 태스크는 `authjs-admin-ui-phase-2-expansion-plan.md`를 본다.
