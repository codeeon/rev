...
| 파일명 | 성격 | 작성 스타일 |
|--------|------|------------|
| `engine-v41-production-design.md` | 확정 스펙 / Source of Truth | 간결한 계약서 |
| `engine-migration-plan.md` | 초기 마이그레이션 개요 | 서술형 |
| `engine-logic-migration-plan.md` | PR 단위 실행 계획 (Codex) | 체크리스트형 |
| `engine-refactor-plan.md` | 파일별 코드 diff 계획 (Claude Opus) | diff 코드 포함 |
| `engine-implementation-design.md` | 종합 구현 설계서 (Claude Sonnet) | 테이블 + 수도코드 |
